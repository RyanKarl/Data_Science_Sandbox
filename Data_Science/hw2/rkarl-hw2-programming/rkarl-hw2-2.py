...
#toolz	0.9.0
#wheel	0.33.4
#Partly inspired by https://www.youtube.com/watch?v=ZdKnZ1xKrRA
...
def tree_calculations(data):
    ...
    for column_index in range(n_columns - 1):        # excluding the last column which is the label
        ...
        for values in data:
            if 'Home' in values or 'Away' in values:
                ...
                SPLIT_Home_Or_Away = split_info_calc([home_count, away_count])
                if SPLIT_Home_Or_Away == 0:
                    GR_Home_Or_Away = 0
                else:
                    GR_Home_Or_Away = IG_Home_Or_Away / SPLIT_Home_Or_Away
            ...
            if 'In' in values or 'Out' in values:
                ...
                SPLIT_In_Or_Out = split_info_calc([in_count, out_count])
                if SPLIT_In_Or_Out == 0:
                    GR_In_Or_Out = 0
                else:
                    GR_In_Or_Out = IG_In_Or_Out/SPLIT_In_Or_Out
            ...
            SPLIT_Media = split_info_calc([NBC_count, ABC_count, FOX_count, CBS_count, ESPN_count])
            if SPLIT_Media == 0:
                GR_Media = 0
            else:
                GR_Media = IG_Media / SPLIT_Media
    ...
    d['GR_Home_Or_Away'] = float(GR_Home_Or_Away)
    d['GR_In_Or_Out'] = float(GR_In_Or_Out)
    d['GR_Media'] = float(GR_Media)
...
def IG_Compare(d, data):
    ...
    if max(d["GR_Media"], d["GR_In_Or_Out"], d["GR_Home_Or_Away"]) == d["GR_Media"]:
        data1 = data.copy()
        data2 = data.copy()
        data3 = data.copy()
        data4 = data.copy()
    ...
    elif max(d["GR_Media"], d["GR_In_Or_Out"], d["GR_Home_Or_Away"]) == d["GR_In_Or_Out"]:
         data6 = data.copy()
         data7 = data.copy()
...
def decision_tree_algorithm(df, counter=0, max_depth=3):
    ...
    if (check_purity(data)) or (counter == max_depth) or (data_dict["GR_Media"] + data_dict["GR_In_Or_Out"] + data_dict["GR_Home_Or_Away"]) == 0:
...
print("Level 0 Question: " + str(q0))
print("Level 1 Answers: " + str(a0))
print("Level 2 Answers: " + str(a1))
print("Level 3 Answers: " + str(a2))
print("Level 4 Answers: " + str(a3))
print("\nPredictions: ")
