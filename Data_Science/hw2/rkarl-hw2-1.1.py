...
'''
def H(a, b, a_win, b_win, a_lose, b_lose):
    a = float(a)
    b = float(b)
    a_win = float(a_win)
    a_lose = float(a_lose)
    ...
    return (((a/(a+b))*(-(((a_win/a)*math.log((a_win/a), 2))+((a_lose/a)*math.log((a_lose/a), 2)))))+((b/(a+b))*(-(((b_win/b)*math.log((b_win/b), 2))+((b_lose/b)*math.log((b_lose/b), 2))))))
'''
...
H_In_Or_Out = H_Home_Or_Away = H_Media = H_NBC = H_ABC = H_FOX = H_CBS = H_ESPN = IG_Media = IG_In_Or_Out = IG_Home_Or_Away = 0
...
def Tree_Calculations(df):
    ...
    if 'Is_Home_or_Away' in df.columns:
        #H_Home_Or_Away = H(home_count, away_count, home_win_count, away_win_count, home_lose_count, away_lose_count);
        ...
        #print(H_Home_Or_Away)
        #print(IG_Home_Or_Away)

    if 'Is_Opponent_in_AP25_Preseason' in df.columns:
        #H_In_Or_Out = H(in_count, out_count, in_win_count, out_win_count, in_lose_count,  out_lose_count);
...
def IG_Compare(IG_Media, IG_In_Or_Out, IG_Home_Or_Away, dataframe):
    print(IG_Media, IG_In_Or_Out, IG_Home_Or_Away)
...
print(tree_dict)
branch1 = IG_Compare(tree_dict["IG_Media"], tree_dict["IG_In_Or_Out"], tree_dict["IG_Home_Or_Away"], df)
