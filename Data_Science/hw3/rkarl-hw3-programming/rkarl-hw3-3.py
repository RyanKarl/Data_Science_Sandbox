...
while True:
    ...
    if df['previous_closest_centroid'].equals(df['closest_centroid']):
        for row in df.itertuples():
             final_SSE +=  min(float(euclidean_distance([row[1], row[2]], centroids[0])),     float(euclidean_distance([row[1], row[2]], centroids[1])), float(euclidean_distance([row[1], row[2]], centroids[2]))) ** 2
